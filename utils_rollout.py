...
def generate_responses(assistant_gen_client, eval_client, sample, conversation, group_size, max_tokens=5000):
    # Schedule all generation jobs in batch
    jobs = [{"conversation": conversation, "n_responses": 1, "max_tokens": max_tokens} for _ in range(group_size)]
...
def generate_priority_tree_responses(assistant_gen_client, eval_client, sample, conversation, num_responses):

    T = time.time()
    resp = assistant_gen_client.build_priority_tree(conversation, max_num_responses=num_responses)
    job_id = resp["job_id"]

    job_status = assistant_gen_client.wait_for_job_completion(job_id)
    full_tree = job_status["tree"]
